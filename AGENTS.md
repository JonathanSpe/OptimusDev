...
- Status colours also carry verdicts on the Analysis surface — except on `/analyse/snapshot`, which deliberately runs without them (see the colour policy comment in `features/analysis/components/analysis-board.tsx`).
