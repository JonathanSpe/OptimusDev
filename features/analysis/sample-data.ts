...
/*
 * ⚠️ HIER LAGEN DER TYP Supplement UND sampleSupplements. Beide sind weg:
 * die Form steht jetzt in contracts/supplement.ts, die Daten in
 * data/mock/supplements.ts.
 *
 * Der Grund war die Empfehlungsseite. Sie braucht Praeparate, die NICHT
 * eingenommen werden, ein Bild, einen Preis und die Abo-Zugehoerigkeit — und
 * damit war der Typ keine Angelegenheit der Analyse mehr, sondern eine
 * gemeinsame Sache zweier Oberflaechen. Genau dafuer gibt es contracts/.
 *
 * Die Entscheidung im Kopf dieser Datei ("Analyse-Typen bleiben vorerst hier")
 * gilt fuer den SCORE unveraendert weiter: der haengt an der Score-Formel, und
 * die ist offen. Ein Praeparat haengt an keiner offenen Formel.
 *
 * Die REGELN zu Praeparaten bleiben in rules.ts. Ein Vertrag beschreibt die
 * Form, kein Urteil.
 */
